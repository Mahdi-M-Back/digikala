...
def checkout(request):
    ...
    if request.user.is_authenticated:
        try:
            shipping_instance = ShippingAddress.objects.get(user__id=request.user.id)
        except ShippingAddress.DoesNotExist:
            shipping_instance = None

        shipping_form = ShippingForm(request.POST or None, instance=shipping_instance)
...
def confirm_order(request):
    if request.POST:
        ...
        messages.success(request , " برو بچه پرو' برو بچه پرو")
...
def process_order(request):
    if request.POST:
        ...
        messages.success(request , 'ای بابا برو توی یکی دیگه')
        return redirect('home')