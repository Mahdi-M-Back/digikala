...
class ShippingAddress(models.Model):
    ...
    shipping_old_cart = models.CharField(max_length=500 , blank=True , null=True)
    ...
    def save(self , *args , **kwargs):
        if self.pk:
            ...
            if old_status != self.status:
                self.last_update = jdatetime.datetime.now().strftime('%Y-%m-%d %H:%M:%S')
...
class OrderIthem(models.Model):
    ...
    def __str__(self):
        if self.user is not None:
            return f'order Ithem - {str(self.id)} - for {self.user}'
        else:
            return f'order Ithem - {str(self.id)}'
