...
from payment.models import ShippingAddress , Order , OrderIthem
...
def login_user(request):
    if request.method == "POST":
        ...
        if user is not None:
            ...
            messages.success(request , ('نام یا رمزعبور اشتباه است'))
...
def search(request):
    if request.method == 'POST':
        ...
        if not searched:
            messages.success(request , 'ای بابا زدی تو هدف خالی')
...
def user_orders(request):
    if request.user.is_authenticated:
        ...
        messages.success(request ," you don't have anythings" )
        return redirect("home")
...
def order_details(request, pk):
    ...
    if request.user.is_authenticated:
        ...
        items = OrderIthem.objects.filter(order=pk)
        context = {
            'order': order,
            'items': items,
        }
